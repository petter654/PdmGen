...
public class PdmService {
    ...
    public String gerarDescricaoLonga(String classe, Map<String, String> valores) { List<String> atributosOrdenados = this.templatesCarregados.get(classe.toUpperCase()); if (atributosOrdenados == null) return "ERRO: Classe não encontrada."; String valoresConcatenados = atributosOrdenados.stream().map(attr -> attr + ": " + valores.getOrDefault(attr, "").toUpperCase()).collect(Collectors.joining("; ")); return classe.toUpperCase() + "; " + valoresConcatenados; }
